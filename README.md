...
- **K3s Documentation**: [Lightweight Kubernetes Deployment Guide](https://docs.k3s.io/)
...
1. Place the YOLOv5 model on the host node:
   ```bash
   mkdir -p /opt/smartfield/models
   cp yolov5su.pt /opt/smartfield/models/
   ```
...
2. Configure system settings in `config.toml`:
   ...
   **IMPORTANT**: Update the camera-trap location coordinates precisely. The drone uses these coordinates to navigate to detection sites. Incorrect values will cause the drone to fly to the wrong location.
   ...
   Also update the MQTT broker address, network settings, and service endpoints as needed.

3. Install K3s and deploy all services via Helm:
   ```bash
   make install
   make deploy
   ```
...
#### Step 4: Helm Release Overview

Inspect the deployed release and its resources:
```bash
helm list -n smartfield
helm status smartfield -n smartfield
```

To override any default value without editing `values.yaml`:
```bash
helm upgrade smartfield charts/smartfield -n smartfield \
  --set config.topic.lat=40.123 \
  --set config.topic.lon=-83.456
```
...
- ✓ Helm-managed release with per-value overrides
- ✓ Centralized log storage via PVC at `/var/log/smartfield/`
...
- Monitor detection logs in `/var/log/smartfield/` to tune sensitivity
...
1. **View all service logs**:
   ```bash
   kubectl -n smartfield logs -f --all-containers --prefix=true --selector='app in (smartfields,openpasslite,wildwings,mqtt-subscriber)'
   ```
...
3. **Filter logs by time**:
   ```bash
   kubectl -n smartfield logs --since=30m deployment/smartfields
   ```

4. **Export logs for analysis**:
   ```bash
   kubectl -n smartfield logs deployment/smartfields > system-logs-$(date +%Y%m%d).txt
   ```
...
def tail_logs(service_name, lines=50):
    cmd = f"kubectl -n smartfield logs --tail {lines} deployment/{service_name}"
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    return result.stdout
```

### Troubleshooting
- **Logs not appearing**: Check if pod is running with `make status`
- **Permission denied**: Ensure kubeconfig is accessible (`export KUBECONFIG=/etc/rancher/k3s/k3s.yaml`)
...
1. Ensure drone is connected and WildWings pod is running:
   ```bash
   kubectl -n smartfield get pods -l app=wildwings
   ```

2. Access the SmartField API to publish a test detection:
   ```bash
   curl -X POST "http://localhost:2188/initiate_pipeline?lat=40.0&lon=-83.0&camid=test-01"
   ```
...
4. Check mission status via OpenPassLite:
   ```bash
   curl http://localhost:2177/mission_status
   ```
...
- **Video not recording**: Check storage space on the `smartfield-media` PVC
...
4. Rebuild the image and restart the pod:
   ```bash
   kubectl rollout restart deployment/mqtt-subscriber -n smartfield
   ```
...
- **Connection refused**: Check MQTT broker pod is running with `kubectl -n smartfield get pods -l app=mosquitto`
...
- **Privileged Pod**: Direct hardware access for drone networking and video devices
...
5. WildWings Executes Flight (40-50 seconds)
                  ↓
6. Video Captured & Stored (/app/media/)
                  ↓
...
**Pod Networking**: `openpasslite` and `wildwings` use `hostNetwork: true` with `dnsPolicy: ClusterFirstWithHostNet` for:
- Direct hardware access to the drone's WiFi network (192.168.42.x)
- Low-latency drone control without network address translation
- K3s DNS still functional alongside host networking
...
4. **Infrastructure as Code**: Helm chart under `charts/` enables reproducible, value-driven deployments
...
- [K3s Documentation](https://docs.k3s.io/) - Lightweight Kubernetes for edge deployments
- [Helm Documentation](https://helm.sh/docs/) - Kubernetes package management and value overrides
...
- **OS**: Linux kernel 5.x+ (Ubuntu 22.04+ recommended)
