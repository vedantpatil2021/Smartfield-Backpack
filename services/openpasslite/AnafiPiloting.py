...
class AnafiPiloting:
	...
	def takeoff(self, queue = False):
		...
		'''
		if queue == False:
			if not self.drone(TakeOff() >> FlyingStateChanged(state="hovering", _timeout=5)).wait().success():
				raise RuntimeError("Takeoff failed: drone did not reach hovering state")
			print("------ TAKEOFF ------")
		else:
	...
	def land(self, queue = False):
		...
		'''
		if queue == False:
			if not self.drone(Landing()).wait().success():
	...
	def wait_until_state(self, state_type, state, timeout = None):
		...
		if timeout == None:
			if state_type == "move_by":
				self.add_action("FlyingStateChanged('{}')".format(state))
			else:
				self.add_action("moveToChanged(status='{}')".format(state))
		else:
			if state_type == "move_by":
				self.add_action("FlyingStateChanged('{}', _timeout={})".format(state, timeout))
			else:
				self.add_action("moveToChanged(status='{}', _timeout={})".format(state, timeout))
	...
	def move_by(self, x, y, z, angle, wait = False, queue = False):
		...
		if queue == False:
			if wait == True:
				if not self.drone(moveBy(x, y, z, angle) >> FlyingStateChanged("hovering")).wait().success():
					raise RuntimeError(f"moveBy({x}, {y}, {z}, {angle}) failed")
			else:
		...
		if queue == False:
			if wait == True:
				if not self.drone(
					...
					raise RuntimeError(f"moveTo lat={lat}, lon={lon} failed")
			else:
				self.drone(
					moveTo(latitude=lat,longitude=lon,altitude=alt,orientation_mode=orientation_mode,heading=heading)
				)
	...
	def cancel_move_by(self):
		...
		if not self.drone(CancelMoveBy()).wait():
			raise RuntimeError("CancelMoveBy failed")
	...
	def cancel_move_to(self):
		...
		if not self.drone(CancelMoveTo()).wait():
			raise RuntimeError("CancelMoveTo failed")
